...
function updateBodyDetectionIndicator(isVisible) {
  ...
  if (detectionIndicator) {
    if (isVisible) {
      detectionIndicator.className = 'body-status-badge full-body-visible'; // Correct class name from HTML
      detectionIndicator.textContent = 'เห็นทั้งตัว'; // Set text directly
    } else {
      detectionIndicator.className = 'body-status-badge full-body-hidden'; // Correct class name from HTML
      detectionIndicator.textContent = 'ไม่เห็นทั้งตัว'; // Set text directly
    }
  }
}
